...
public class JdbcPortfolioRepository implements PortfolioRepository {
	...
	public int save(Portfolio portfolio, Client client) {
		...
		return jdbcTemplate.update(
				"insert into  portfolio (clientId,total,loyalty,balance,commissions,freeTrades) values(?,?,?,?,?,?)",
				portfolio.getClientId(), portfolio.getTotal(), portfolio.getLoyalty(), portfolio.getBalance(),
				portfolio.getCommissions(), portfolio.getFreeTrades()) + first;
	...
	public List<Portfolio> findAll() {
		return jdbcTemplate.query(
				"select p.portfolioId,p.total,p.loyalty,p.balance,p.commissions,p.freeTrades, c.firstName, c.lastName  from portfolio p, client c where c.clientId = p.clientId",
	...
	public Optional<Portfolio> tradeEquities(PortfolioUpdate update, List<StockQuote> quotes) {
		...
		for (StockQuote quote : quotes) {
			logger.info("Putting " + quote.getSymbol() + " and " + quote.getPrice() + " in quoteDict");
		...
		if (free > 0) { // use a free trade if available
			...
			logger.info("Using free trade for " + currentPortfolio.getPortfolioId());
			...
			currentPortfolio.setCommissions(commissions);
		}
		...
		if ((stocks != null) && (stocks.size() > 0)) {
			for (Stock stock : stocks) {
				if (update.getSymbol().equals(stock.getSymbol())) {
					...
					balance = balance.subtract(quoteDict.get(stock.getSymbol()).multiply(new BigDecimal(update.getShares()).setScale(2, BigDecimal.ROUND_HALF_EVEN)));
					newHolding = false;
				}
				stock.setLastQuoted(currentTimestamp);
				...
				if (stock.getShares() > 0) {
					jdbcTemplate.update(
							...
							stock.getLastQuoted(), stock.getPortfolioId(), stock.getSymbol());
		...
		if (newHolding) {
			...
			balance = balance.subtract(quoteDict.get(update.getSymbol()).multiply(new BigDecimal(update.getShares()).setScale(2, BigDecimal.ROUND_HALF_EVEN)));
			logger.info("Just before insert symbol is " + update.getSymbol());
			Stock stock = new Stock(update.getPortfolioId(), update.getSymbol(), update.getShares(), price, total,
					commission, currentTimestamp);
			jdbcTemplate.update(
					...
					stock.getCommission(), stock.getLastQuoted());
		...
		//public TradeHistory(int portfolioId, String symbol, int shares, BigDecimal price,
		//		BigDecimal commission, long timeOfTrade ) {
		TradeHistory lastTrade = new TradeHistory(update.getPortfolioId(), update.getSymbol(),update.getShares(), quoteDict.get(update.getSymbol()),commission,currentTimestamp.getTime());
		...
		jdbcTemplate.update(
				...
				currentPortfolio.getTotal(), currentPortfolio.getLoyalty(), currentPortfolio.getBalance(), currentPortfolio.getCommissions(),
				currentPortfolio.getFreeTrades(), currentPortfolio.getPortfolioId());
